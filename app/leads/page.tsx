...
export default function LeadsPage() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const excelData = leadsData.map((lead: any) => ({
          ...
          "Hospitalized/Nursing/Oxygen/Cancer Assistance":
            lead.hospitalized_nursing_oxygen_cancer_assistance ? "Yes" : "No",
          "Organ Transplant/Terminal Condition":
            lead.organ_transplant_terminal_condition ? "Yes" : "No",
          "AIDS/HIV/Immune Deficiency": lead.aids_hiv_immune_deficiency
            ? "Yes"
            : "No",
          "Diabetes Complications/Insulin": lead.diabetes_complications_insulin
            ? "Yes"
            : "No",
          "Kidney Disease/Multiple Cancers":
            lead.kidney_disease_multiple_cancers ? "Yes" : "No",
          "Pending Tests/Surgery/Hospitalization":
            lead.pending_tests_surgery_hospitalization ? "Yes" : "No",
          "Angina/Stroke/Lupus/COPD/Hepatitis":
            lead.angina_stroke_lupus_copd_hepatitis ? "Yes" : "No",
          "Heart Attack/Aneurysm/Surgery": lead.heart_attack_aneurysm_surgery
            ? "Yes"
            : "No",
          "Cancer Treatment (2 years)": lead.cancer_treatment_2years
            ? "Yes"
            : "No",
          "Substance Abuse Treatment": lead.substance_abuse_treatment
            ? "Yes"
            : "No",
          "Cardiovascular Events (3 years)": lead.cardiovascular_events_3years
            ? "Yes"
            : "No",
          "Cancer/Respiratory/Liver (3 years)":
            lead.cancer_respiratory_liver_3years ? "Yes" : "No",
          "Neurological Conditions (3 years)":
            lead.neurological_conditions_3years ? "Yes" : "No",
  ...
  const handleDeleteLead = async (leadId: number, leadName: string) => {
    ...
    try {
      ...
      alert(
        `Failed to delete lead: ${error.response?.data?.message || "Unknown error"}`,
      );
    }
  };
            ...
            >
              <div>
                ...
                <p className="page-subtitle">
                  Manage and track all patient leads
                </p>
              </div>
                        ...
                        <tr key={lead.id}>
                          ...
                          <td>
                            ...
                            <div className="text-sm text-muted">
                              DOB:{" "}
                              {lead.date_of_birth ? lead.date_of_birth : "N/A"}
                            </div>
                          </td>
                          <td>
                            <div>{lead.phone}</div>
                          ...
                          <td>
                            <span
                              className={`badge ${getStatusBadgeClass(lead.status_id)}`}
                              style={{
                                padding: "4px 12px",
                                borderRadius: "12px",
                                fontSize: "12px",
                              ...
                              {(currentUser?.role_id === 1 || currentUser?.role_id === 6) && (
                                <button
                                  onClick={() =>
                                    handleDeleteLead(
                                      lead.id,
                                      `${lead.first_name}${lead.middle_initial ? " " + lead.middle_initial : ""} ${lead.last_name}`,
                                    )
                                  }
                                  className="btn-icon"
                                  title="Delete Lead"
                ...
                >
                  ...
                  >
                    <div style={{ fontSize: "14px", color: "var(--gray-600)" }}>
                      Showing {startIndex + 1} to {endIndex} of {totalLeads}{" "}
                      leads
                    </div>
                    <div
                      style={{
                        ...
                        pageButtons.push(
                          <button
                            ...
                            style={{
                              ...
                              fontSize: "14px",
                              fontWeight:
                                currentPage === totalPages ? "600" : "500",
                              minWidth: "40px",
                            }}
                          >
                            {totalPages}
